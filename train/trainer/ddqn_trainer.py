...
class DDQNTrainer:
    ...
    def execute_environment_step(self, actions, current_states):
        ...
        total_step_profit = env_step_info[6]
    ...
    def run_single_episode(self):
        ...
        current_states = self.env.get_observations()
    ...
    def run_episode_with_reward_adjustment(self):
        ...
        for t in range(self.max_episode_length):
            ...
            total_profit += total_step_profit
        ...
        total_completed = sum(1 for m in self.env.missions if m.get_status() == 2)
        logger.info(
            f"---> Rewards from DDQN: Total Completed Tasks: {total_completed}, Total Profit: {total_profit}"
        )
        ...
        while (self.env.action_memory == 0).any():
            for agent_idx, state_key in enumerate(states):
                ...
                if agent.epsilon > self.env.rng.random() or iteration > max_free_select:
                    action = self.env.rng.integers(0, agent.action_dim)
                else:
                ...
                if self.env.action_memory[action]:
                    mem[2].append(-0.01 * self.env.ideal_avg_reward)
                    mem[4].append(-1)
                    continue
    ...
    # step_ma
    def run_multi_action_step(self):
        """
