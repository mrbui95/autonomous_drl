...
from simulation.envs.utils import get_ideal_expected_reward
...
class Environment(gym.Env):
    ...
    def initialize_missions(self):
        ...
        def create_single_mission(mission_data):
            ...
            mission.set_depend_mission(mission_data["dependencies"])
    ...
    # step
    def step_env(self, actions_dict, agents=None, states=None, call_from="DRL"):
        """
        ...
        for idx, vehicle in enumerate(self.vehicles):
            ...
            total_completed_tasks += vehicle.get_total_completed()
            total_step_profit += vehicle.get_total_profit()
            if idx in rewards:
            ...
            if vehicle.is_on_time():
                intime = True
        logger.info(
            f"Tổng profit hệ thống: {total_system_profit}, total_profit_vehicle: {total_step_profit}, nhiệm vụ hoàn thành: {total_completed_tasks}"
        )
        ...
        if done or count_done == self.env_data['num_vehicles']:
            logger.debug(f"done: {done}, count_done: {count_done} action_memory: ${self.action_memory}")
            self.done = True
        else:
            self.done = False
        ...
        if self.verbose:
            logger.info(
                f"---> Rewards from {call_from}: {rewards}, Total Completed Tasks: {total_completed_tasks}, Total Profit: {total_system_profit}"
            )

        logger.debug("=== Kết thúc step_env ===\n")

        return obs, rewards, self.done, truncated, done_process_info_list, action_taken, total_step_profit

    # step_ma
    def step_multi_agent(self, actions, call_from="DRL"):
        """
        ...
        for vehicle in self.vehicles:
            vehicle.assign_missions(actions, self.missions, has_order_tuple=True)
        for vehicle in self.vehicles:
        ...
        if self.verbose:
            logger.info(
                f"---> Rewards from {call_from}: Total Completed Tasks: {total_completed_tasks}, Total Profit: {total_profit}"
            )
